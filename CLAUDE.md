...
- **`kernel/archive/rockchip64-7.3/`** — same-named files here **replace** Armbian's core patches of that name (the patcher keys patches by filename, user root last). Today it holds one: a tab-corrected copy of Armbian's Orange Pi 5 ES8388 patch, whose space-indented hunk fails against 7.3-rc1 and, being fatal, killed the kernel build. Unrelated to this board; delete it when Armbian fixes theirs. An *empty* override is not a way to drop a patch — the patcher raises on a file with no valid patch.
