...
- **`kernel-overrides/archive/rockchip64-7.3/`** — files here are copied **over** Armbian's core patch of the same name by the extension at the end of config (a same-named file under `userpatches/kernel/` does *not* win: the patcher keys patches by filename and iterates the core root last, so core shadows user — verified 2026-09-02). Today it holds one: a tab-corrected copy of Armbian's Orange Pi 5 ES8388 patch, whose space-indented hunk fails against 7.3-rc1 and, being fatal, killed the kernel build. Unrelated to this board; delete it when Armbian fixes theirs. An *empty* file is not a way to drop a patch — the patcher raises on it.
