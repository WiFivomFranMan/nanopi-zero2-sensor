...
- **Onboard Ethernet**: `overlay/etc/systemd/network/10-ethernet.network` matches `end1` — confirmed on real hardware (kernel log: `rk_gmac-dwmac ffbe0000.ethernet end1: renamed from eth0`). There's only one physical GMAC/PHY on this board; it lands on `end1` rather than `end0` because udev's onboard naming (`ID_NET_NAME_ONBOARD`) reads the index straight from the devicetree's `ethernet1` alias, which the board's devicetree inherits from Rockchip's shared RK356x dtsi (where that label distinguishes it from RK3568's second GMAC) rather than renumbering it to `ethernet0`. The unit sets `DHCP=yes` plus a statically-applied `172.16.0.1/24` — systemd-networkd applies `Address=` unconditionally and immediately, alongside any DHCP lease, so the address is always present, not just as a fallback when DHCP is unavailable. This is deliberate for now: it keeps a single unit reachable at a known address regardless of DHCP, at the cost that multiple units on the same network segment will all claim `172.16.0.1` simultaneously (a real conflict if more than one sensor is deployed on one LAN).
