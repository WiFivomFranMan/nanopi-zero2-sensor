...
- **Wi-Fi scanning access**: `overlay/etc/sudoers.d/pi-nopasswd-wifi` grants the `pi` user passwordless sudo for exactly `iw`, `ip`, and `tcpdump` — the minimum needed for Wi-Fi scanning/packet capture tooling without full root, on top of `pi`'s normal (password-prompted) `sudo` group membership.
